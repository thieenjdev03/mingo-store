...
import {
  ...
  fetchNavCategories,
  localNavBrands,
} from "@/features/catalog/nav-data";
...
const SOCIALS = [
  ...
  },
  {
    ...
    icon: "/assets/mingo/home/instagram.svg",
  },
  {
    label: "TikTok",
    href: "https://www.tiktok.com",
    icon: "/assets/mingo/home/tiktok.svg",
  },
];
...
export async function SiteFooter() {
  ...
  const linkClass =
    "block py-0.5 text-[14px] uppercase leading-[26px] text-[#563e2b] transition-colors hover:text-primary lg:text-[16px] lg:leading-7";
    ...
    <footer className="bg-white text-[#563e2b]">
      <div className="mx-auto grid h-auto max-w-[1440px] items-start gap-10 px-5 py-10 sm:grid-cols-2 sm:px-8 lg:grid-cols-[minmax(0,270px)_120px_120px_155px_1fr] lg:gap-[31px] lg:py-6 min-[1504px]:px-0">
        <div>
          ...
          <address className="text-[12px] not-italic leading-[26px] text-[#563e2b] lg:text-[14px] lg:leading-7">
        ...
        <div>
          <h3 className="mb-3 text-[14px] font-bold uppercase leading-8 lg:text-[16px]">
            {t("aboutTitle")}
          </h3>
          <ul>
            <li>
        ...
        <div>
          ...
          <ul>
            {lines.map((line) => (
              <li key={line.slug}>
                <Link href={`/categories/${line.slug}`} className={linkClass}>
                  {line.name}
                </Link>
              </li>
            ))}
