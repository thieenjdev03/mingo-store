    ...
    <div className="fixed inset-0 z-[70]" role="presentation">
      ...
      <aside aria-label={t('drawerTitle')} aria-modal="true" role="dialog" className="absolute right-0 top-0 flex h-full w-full max-w-[440px] flex-col bg-card shadow-2xl">
        <header className="flex items-center justify-between border-b border-border px-5 py-5 sm:px-7">
          ...
          <button type="button" onClick={cart.closeDrawer} aria-label={t('close')} className="rounded-full p-2 hover:bg-muted">
            <X className="size-5" aria-hidden="true" />
          </button>
        </header>

        {cart.errorMessage ? (
          <div className="mx-5 mt-4 flex gap-3 rounded-lg bg-destructive/10 p-3 text-sm font-semibold text-destructive sm:mx-7" role="alert">
            <span className="flex-1">{cart.errorMessage}</span>
            <button type="button" onClick={cart.dismissError} aria-label={t('dismissError')}>
        ...
        {cart.isLoading ? (
          ...
          <div className="flex flex-1 flex-col items-center justify-center px-8 text-center">
            ...
            <Link href="/products" onClick={cart.closeDrawer} className="mt-6 inline-flex h-11 items-center justify-center rounded-lg bg-primary px-6 text-sm font-bold text-primary-foreground transition-colors hover:bg-primary-dark">
              {t('continueShopping')}
            </Link>
          </div>
        ) : (
          <>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3 flex items-center justify-between gap-3">
                      <div className="flex items-center rounded-md border border-border">
                        <button type="button" aria-label={t('decrease')} disabled={cart.isMutating || item.quantity <= 1} onClick={() => void cart.setQuantity(item.id, item.quantity - 1)} className="p-1.5 disabled:opacity-35"><Minus className="size-3.5" aria-hidden="true" /></button>
                        <span className="min-w-7 text-center text-sm font-semibold">{item.quantity}</span>
                        <button type="button" aria-label={t('increase')} disabled={cart.isMutating || item.quantity >= item.product.stock} onClick={() => void cart.setQuantity(item.id, item.quantity + 1)} className="p-1.5 disabled:opacity-35"><Plus className="size-3.5" aria-hidden="true" /></button>
                      </div>
                      <p className="font-bold text-primary">{fCurrencyVND(item.lineTotal)}</p>
                    </div>
                  </div>
                </article>
              ))}
            ...
            <footer className="space-y-3 border-t border-border px-5 py-5 sm:px-7">
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <Link href="/cart" onClick={cart.closeDrawer} className="inline-flex h-11 items-center justify-center rounded-lg border-2 border-foreground px-4 text-sm font-bold transition-colors hover:bg-accent">{t('viewCart')}</Link>
                {cart.valid ? (
                  <Link href="/checkout" onClick={cart.closeDrawer} className="inline-flex h-11 items-center justify-center rounded-lg bg-primary px-4 text-sm font-bold text-primary-foreground transition-colors hover:bg-primary-dark">{t('checkout')}</Link>
                ) : (
                  <span aria-disabled="true" className="inline-flex h-11 cursor-not-allowed items-center justify-center rounded-lg bg-muted px-4 text-sm font-bold text-muted-foreground">{t('checkout')}</span>
                )}
              </div>
            </footer>
          </>
