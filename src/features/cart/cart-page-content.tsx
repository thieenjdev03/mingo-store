    ...
    <div className="bg-ivory py-12 sm:py-16 lg:py-20">
      <div className="mx-auto max-w-[1100px] px-5 sm:px-8">
        <div className="flex items-end justify-between gap-4">
          ...
          {cart.items.length ? <button type="button" disabled={cart.isMutating} onClick={() => void cart.clear()} className="text-sm font-bold text-muted-foreground hover:text-destructive disabled:opacity-50">{t('clear')}</button> : null}
        </div>
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_340px]">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
                      <div className="flex items-center rounded-lg border border-border">
                        <button type="button" aria-label={t('decrease')} disabled={cart.isMutating || item.quantity <= 1} onClick={() => void cart.setQuantity(item.id, item.quantity - 1)} className="p-2.5 disabled:opacity-35"><Minus className="size-4" aria-hidden="true" /></button>
                        <span className="min-w-9 text-center font-semibold">{item.quantity}</span>
                        <button type="button" aria-label={t('increase')} disabled={cart.isMutating || item.quantity >= item.product.stock} onClick={() => void cart.setQuantity(item.id, item.quantity + 1)} className="p-2.5 disabled:opacity-35"><Plus className="size-4" aria-hidden="true" /></button>
                      </div>
                      <p className="text-lg font-bold text-primary">{fCurrencyVND(item.lineTotal)}</p>
                    </div>
                  </div>
                </article>
              ))}
            ...
            <aside className="h-fit rounded-xl bg-card p-6 shadow-sm">
              ...
              <div className="mt-6 flex items-center justify-between border-b border-border pb-5">
                ...
                <span className="text-xl font-bold text-primary">{fCurrencyVND(cart.subtotal)}</span>
              </div>
              ...
              {cart.valid ? (
                ...
                <span aria-disabled="true" className="mt-6 flex h-12 cursor-not-allowed items-center justify-center rounded-lg bg-muted px-5 text-sm font-bold text-muted-foreground">{t('checkout')}</span>
              )}
