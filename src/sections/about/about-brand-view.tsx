...
function ContactPill({ href, label, icon, compact = false }: ContactPillProps) {
  ...
  return (
    <a
      href={href}
      className={`inline-flex min-h-10 w-full items-center gap-2 rounded-full border-2 border-foreground/70 px-4 py-2 font-semibold transition-colors hover:border-primary hover:text-primary ${compact ? "text-xs" : "text-sm"}`}
    >
    ...
    <div className="bg-ivory">
      ...
      >
        <div className="flex min-h-[360px] flex-col bg-coral px-6 py-12 text-center sm:px-12 lg:min-h-[520px] lg:px-16 lg:py-14">
          ...
          <div className="flex flex-1 items-center justify-center">
            <p className="max-w-xl text-base font-bold leading-7 text-primary-foreground sm:text-lg">
              {t("story.belief")}
            </p>
          </div>
        </div>
      ...
      >
        <div className="text-center">
          ...
          <p className="mt-9 inline-block border-b-2 border-primary text-xl font-bold text-primary sm:text-2xl">
            {t("office.tab")}
          </p>
        </div>
