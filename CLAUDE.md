...
All HTTP requests go through `src/lib/api/fetcher.ts` (`customFetch`, the orval mutator). There is no BFF/proxy layer — the browser calls the backend directly via `NEXT_PUBLIC_API_URL`; server components/route handlers call it via `API_URL` (`buildServerApiUrl` in `src/lib/api/server-url.ts`). Both are required env vars (see `.env.example`); a missing one surfaces as `ApiError(503, "Backend API is not configured")`. `customFetch` attaches `Authorization: Bearer` from `src/lib/auth/token.ts` (JWT lives in `localStorage`, for customers and admins alike) when a token is present; only the **refresh-token interceptor** is still a TODO, so a 401 currently just fails rather than retrying. Write endpoints (POST/PATCH/DELETE on products/categories/colors/sizes/files) require an admin bearer token on the backend — the public storefront only reads, but `/admin` (below) does write and relies on this header. Backend CORS is currently wide open (`CORS_ORIGIN=*`); if that ever gets restricted to specific origins, remember to keep this frontend's origin(s) on the list.
...
| Session | `src/lib/auth/token.ts` | same token key (`src/lib/auth/token.ts`) so `customFetch` picks it up, plus a separate `mingo-admin-user` entry in `src/lib/admin/auth.ts` for the role guard |

Feature logic lives in `src/features/admin/*`, shared chrome in `src/components/admin/*` (`admin-shell.tsx`, `admin-guard.tsx`, and its own `ui/` primitives — separate from `src/components/ui/`). Every `/admin/**` page is a client component (`'use client'`) — there is no server-rendered admin data fetch — so `AdminGuard` (checks `/me` via `src/lib/admin/session-client.ts` on mount) is the *only* access check; there is no middleware-level gate for `/admin` anymore.
