...
- **`category`/`collections` are flat strings, not localized.** `Category`/`Collection` entities are plain `varchar` in the backend, not JSONB — `ProductCategorySummaryDto.name`/`slug` are plain `string`. `ProductResponseDto.collections` is now populated (backend `transformProductForLocale()` maps active `productCollections`), and `toProductDetailView()` reads `collections[0]` into `collectionName`/`collectionSlug`.
